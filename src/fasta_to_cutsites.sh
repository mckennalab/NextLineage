...
inputSites.foreach{site => {
  val sitePattern = ( site + "|" + reverseCompString(site)).r
  ...
  positions.foreach{pos => {
    if (inputRef.slice(pos.start,pos.start+20) ==  site.slice(0,20)) {
      outputArray :+= (inputRef.slice(pos.start,pos.start+23),(pos.start),(pos.start + 17))
      fwd += 1
    }else{
      outputArray :+= (inputRef.slice(pos.start,pos.start+23),(pos.start),(pos.start + 6))
      rev += 1
    }
  }}
}}
