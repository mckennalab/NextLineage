...
def output_stats(input_file_gz: String,output_file: PrintWriter,sample_name: String,guides: Array[String], knownTargets: Array[String]) {
  ...
  val header = stats_file.next()
    ...
    assert(guides.size == targetSites.size)
  var cnt = 0
  var match_count = 0
  var known_count = 0
  var left = 0
  ...
  stats_file.foreach{line => {
    ...
    if ((line contains "MERGED") && (line contains "PASS")) {
      ...
      targetSites.map{k => sp(k).slice(0,23)}.toArray.zipWithIndex.foreach{case(target,index) => {
        ...
        output_file.write(sample_name + "\t" + count + "\t" + guide + "\t" + target + "\t" + outcome + "\t" + edits + "\t" + edit_distance_right_align(guide,target) + "\n")
...
output_file.write("sample\tcount\tguide\ttarget\tclosestMatch\tgtDist\tpalguide\ttarget\toutcome\tpass\ttype\tediting\tnumberEdits\tpositions\n")
