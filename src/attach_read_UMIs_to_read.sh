...
val trimCount = args(3).toInt

val outputReads = new PrintWriter(gos(args(4)))
...
inputReads.foreach{case(read) => {
    ...
    outputReads.write(read(0) + "\n" + barcodePlusUMI + read(1).slice(trimCount,read(0).size) + "\n" + read(2) + "\n" + barcodePlusUMIQual + read(3).slice(trimCount,read(0).size) + "\n")
}}
outputReads.flush()
outputReads.close()